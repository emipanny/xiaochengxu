...
Page({
  ...
  init: function () {
    let that = this;
    let id = that.data.id;
    let url = "https://xcx.envisioneer.cn/customer/getTalks";
    let data = { id: id };
  ...
  send: function () {
    ...
    if (value) {
      let url = "https://xcx.envisioneer.cn/customer/sendTalkText";
      let data = { value, id };
      api.request(url, data)
        .then(function (res) {
          if (res == 1) {
  ...
  upImg: function () {
    let url = "https://xcx.envisioneer.cn/customer/sendTalkImg";
    let that = this;
    let { id } = that.data;
